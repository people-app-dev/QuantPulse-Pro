...
const SUMMARY_TEMPLATES = [
  (date, a) => `全球市场在${date}呈现${a.usdDirection}格局。${a.usdDesc}；亚太市场${a.asiaDesc}；欧洲市场${a.euDesc}。市场参与者可关注：${a.advice}。`,
...
function analyzeMarket(rates) {
  ...
  const advices = ['多元配置有助于分散风险，可关注优质资产回调中的机会', '业绩确定性强的板块在当前环境下关注度较高', '哑铃型策略可在防御与成长之间取得平衡', '逢低分批布局是常见的长期投资方式'];
  ...
  return {
    ...
    conclusion: diff > 0 ? '短期外部压力尚存，但国内基本面改善趋势未变' : '内外环境共振改善，市场结构性机会增多',
    ...
    positionTip: diff > 0 ? '防御性配置在当前环境下受到更多关注' : '科技和消费板块近期关注度提升',
    ashareShort: diff > 0 ? '震荡整理，结构性行情延续' : '交投活跃，市场信心修复',
    keyTip: diff > 0 ? '高股息和必选消费等防御方向受关注度较高' : '科技成长和消费复苏是近期市场关注的方向',
  };
}
